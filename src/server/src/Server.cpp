...
Server::Server(int port) : port(port), serverSocket(0) {
	...
	// Creating our thread pool with 5 threads
...
void Server::initialize() {
	...
	do {
		...
		if (strcmp(exitCommand.c_str(), "Exit") == 0) {
			// Also terminating our thread pool
...
void *acceptClient(void *obj) {
	try {
		...
		while (true) {
			...
			// Sending the task to our thread pool to execute
			Task *handleClientTask = new Task(handleClient, info);
			server->threadPool->addTask(handleClientTask);
			delete handleClientTask;
		}
